...
export const addEdges = () => {
  for (let i = 0; i < nodes.length; i++) {
    ...
    const distanceFromCenter = Math.sqrt((row - gridSize / 2) ** 2 + (col - gridSize / 2.1) ** 2);

    if (distanceFromCenter < gridSize / 2.1) {
      if (col < gridSize - 1 && Math.random() < .7) {
        addEdge(i, i + 1);
      }

      if (row < gridSize && Math.random() < .7) {
...
export const findNearestNode = (pos: position): number => {
  const { x, y } = pos;
  let nearestNode = -1;
